...
void wifi_init(const char *wifi_ssid, const char *wifi_pass, const char *hostname)
{
    ...
    // Network infrastructure should already be initialized by network_infrastructure_init()
    // You can comment out this 2 lines below
    ESP_ERROR_CHECK(esp_netif_init());
    ESP_ERROR_CHECK(esp_event_loop_create_default());
...
esp_err_t ethernet_init_for_nerdaxe(const char* hostname) {
    ...
    vTaskDelay(pdMS_TO_TICKS(500));  // Give W5500 time to stabilize
    return ESP_OK;
}
