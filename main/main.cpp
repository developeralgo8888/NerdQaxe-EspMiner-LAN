...
static void setup_network()
{
    ...
    // Phase 1: Initialize network infrastructure (esp_netif + event loop)
    network_infrastructure_init();

#ifdef CONFIG_ENABLE_ETHERNET
    // Check NVS for network mode preference
    char* network_mode = Config::getNetworkMode();
    MemoryGuard gNetworkMode(network_mode);

    if (strcmp(network_mode, "ethernet") == 0) {
        ESP_LOGI(TAG, "Network mode: Ethernet (from NVS)");
        SYSTEM_MODULE.setNetworkMode(NETWORK_MODE_ETHERNET);

        // Get hostname for Ethernet
        char *hostname = Config::getHostname();
        MemoryGuard gHostname(hostname);

        // Try Ethernet initialization
        ESP_LOGI(TAG, "Initializing Ethernet...");
        esp_err_t eth_err = ethernet_init_for_nerdaxe(hostname);

        if (eth_err == ESP_OK) {
            // Check hardware availability
            bool eth_available = ethernet_is_available();
            SYSTEM_MODULE.setEthAvailable(eth_available);

            if (eth_available) {
                ESP_LOGI(TAG, "W5500 hardware detected, waiting for IP...");
                ...
                int retries = 100;  // 100 x 100ms = 10 seconds
                bool got_ip = false;
                ...
                while (retries-- > 0) {
                    ...
                    if (ethernet_get_ip(ip_buf, sizeof(ip_buf))) {
                        ESP_LOGI(TAG, "Ethernet connected! IP: %s", ip_buf);
                        got_ip = true;

                        // Start REST server
                        start_rest_server(NULL);
                        return;  // SUCCESS - Ethernet working
                    }
                ...
                if (!got_ip) {
                    ESP_LOGW(TAG, "Ethernet timeout (10s), falling back to WiFi");
                }
            } else {
                ESP_LOGW(TAG, "W5500 hardware not detected, falling back to WiFi");
            }
        } else {
            ESP_LOGW(TAG, "Ethernet init failed (%s), falling back to WiFi", esp_err_to_name(eth_err));
        }

        // Fall through to WiFi... (but DON'T save to NVS - keep user preference!)
        ESP_LOGI(TAG, "Switching to WiFi mode for this session (keeping Ethernet preference in NVS)");
        SYSTEM_MODULE.setNetworkMode(NETWORK_MODE_WIFI);
        // REMOVED: Config::setNetworkMode("wifi");  // DO NOT overwrite user's NVS preference!
    }
#endif

    // WiFi Mode (either selected or Ethernet fallback)
...
#ifdef CONFIG_ENABLE_ETHERNET
    // Mark W5500 as available for UI (hardware validation happens on mode switch)
    SYSTEM_MODULE.setEthAvailable(true);
...
extern "C" void app_main(void)
{
	...
	// CRITICAL: Initialize network BEFORE board to avoid SPI bus conflicts
    // Ethernet W5500 uses SPI2, display also uses SPI
    // Network must claim SPI resources first (same order as V3.1)
    setup_network();
