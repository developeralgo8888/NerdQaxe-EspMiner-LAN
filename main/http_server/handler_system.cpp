...
esp_err_t GET_system_info(httpd_req_t *req)
{
    ...
    const char* network_mode_str = (SYSTEM_MODULE.getNetworkMode() == NETWORK_MODE_ETHERNET) ? "ethernet" : "wifi";
...
esp_err_t GET_system_asic(httpd_req_t *req)
{
	...
	// close http connection and prevent it from "hanging"
...
esp_err_t GET_ethernet_status(httpd_req_t *req)
{
    ...
    // Network mode
    doc["networkMode"] = (SYSTEM_MODULE.getNetworkMode() == NETWORK_MODE_ETHERNET) ? "ethernet" : "wifi";
